...
SWEP.PrintName			= "Mercenary Medigun"
SWEP.Slot				= 4
...
SWEP.ViewModel			= "models/weapons/v_models/v_medigun_merc.mdl"
...
SWEP.ShootSound2 = Sound("buttons/button10.wav")
SWEP.ChargedSound = Sound("buttons/button1.wav") 
	...
	if not self.Firing then
		...
		if medigun_trace_condition(tr, self) then
			...
			timer.Create("LoopPlayerAttack1", 0.2, 0, function()
				if not self.Owner:KeyDown(IN_ATTACK) then timer.Stop("LoopPlayerAttack1") return end
...
self.ChargedALoop:Play()
	...
	timer.Create("NowGo!", 0.1, 1, function()
		self:EmitSound( "vo/mercenary_go"..math.random(1,3)..".mp3", 80, 100, 1, CHAN_VOICE)
	end)
